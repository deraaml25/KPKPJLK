...
<?php $component->withAttributes([]); ?>
    <?php $__env->startSection('title', 'Buat Usulan Pemberhentian & SK Kades'); ?>

    <div class="max-w-4xl mx-auto bg-white rounded-card shadow-sm border border-border p-8 mb-8" x-data="{
        kategori: 'pj_kades',
        metode: 'online',
        alasanPjId: '<?php echo e($alasanPj->first()->id ?? ''); ?>',
        alasanPltId: '<?php echo e($alasanPlt->first()->id ?? ''); ?>'
    }">
        ...
        <form action="<?php echo e(route('desa.pjkades.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
                ...
                <template x-if="kategori === 'pj_kades'">
                    <div>
                        <select name="alasan_pemberhentian_id" class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-primary focus:ring-primary shadow-sm" required>
                            <?php $__currentLoopData = $alasanPj; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alasan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($alasan->id); ?>"><?php echo e($alasan->nama); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                ...
                <template x-if="kategori === 'plt_kades'">
                    <div>
                        ...
                        <select name="alasan_pemberhentian_id" x-model="alasanPltId" class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-amber-500 focus:ring-amber-500 shadow-sm" required>
                            <?php $__currentLoopData = $alasanPlt; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alasan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($alasan->id); ?>"><?php echo e($alasan->nama); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                        ...
                        <template x-if="alasanPltId == '<?php echo e($idAlasanPenting); ?>'">
                            <div class="mt-4">
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="nama_pns" id="nama_pns" required value="<?php echo e(old('nama_pns')); ?>"
                            ...
                            <div>
                                ...
                                <input type="text" name="nip" id="nip" required value="<?php echo e(old('nip')); ?>"
                                    class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-primary focus:ring-primary shadow-sm"
                        ...
                        <div>
                            ...
                            <input type="text" name="pangkat" id="pangkat" required value="<?php echo e(old('pangkat')); ?>"
                                class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-primary focus:ring-primary shadow-sm"
                ...
                <template x-if="kategori === 'plt_kades'">
                    <div class="space-y-4">
                        <div>
                            ...
                            <?php if($sekdes): ?>
                                <input type="text" name="nama_plt" id="nama_plt" value="<?php echo e($sekdes->nama); ?>" readonly
            ...
            <div class="flex items-center justify-end gap-3 border-t border-border pt-6">
                <a href="<?php echo e(route('desa.pjkades.index')); ?>"
...
<?php endif; ?><?php /**PATH C:\laragon\www\sidmini\resources\views/desa/pjkades/create.blade.php ENDPATH**/ ?>
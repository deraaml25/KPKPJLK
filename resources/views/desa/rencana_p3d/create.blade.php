<x-app-layout>
    @section('title', 'Input Rencana P3D')
    ...
    <div class="max-w-4xl mx-auto bg-white rounded-card shadow-sm border border-border p-8 mb-8">
        <div class="mb-6 border-b border-border pb-4">
            <h2 class="text-xl font-display font-bold text-ink">Input Data Rencana P3D Baru</h2>
            <p class="text-muted text-sm mt-1">Lengkapi form di bawah untuk melaporkan kekosongan formasi jabatan perangkat desa beserta rencana pelaksanaan penjaringan penggantinya.</p>
        </div>
        ...
        <form action="{{ route('desa.rencana-p3d.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_formasi_kosong" id="jumlah_formasi_kosong" min="1" required value="{{ old('jumlah_formasi_kosong', 1) }}"
                ...
                <div>
                    ...
                    <input type="date" name="rencana_pelaksanaan" id="rencana_pelaksanaan" required value="{{ old('rencana_pelaksanaan') }}"
                        class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-primary focus:ring-primary shadow-sm">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="jabatan_kosong" id="jabatan_kosong" required value="{{ old('jabatan_kosong') }}"
                    class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-primary focus:ring-primary shadow-sm"
            ...
            <div class="mb-6">
                ...
                <textarea name="keterangan" id="keterangan" rows="4"
                    class="w-full text-sm rounded-md border-border text-ink bg-white focus:border-primary focus:ring-primary shadow-sm"
                    placeholder="Tuliskan keterangan mengenai kondisi kekosongan jabatan atau detail rencana lainnya...">{{ old('keterangan') }}</textarea>

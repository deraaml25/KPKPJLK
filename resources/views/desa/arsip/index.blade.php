<x-app-layout>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-muted text-sm mt-1">Daftar surat rekomendasi dari Bupati yang telah terbit untuk desa Anda.</p>
        </div>
    </div>
    ...
    <div class="bg-surface rounded-card border border-border shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-border">
                <thead>
                    <tr class="bg-gray-50">
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">No. Surat Rekomendasi</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Perangkat Desa / Layanan</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">No. Registrasi Ajuan</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Tanggal Terbit</th>
                ...
                <tbody class="divide-y divide-border">
                    @forelse($arsips as $arsip)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-ink">{{ $arsip->ajuan->perangkatDesa->nama }}</div>
                                <div class="text-xs text-muted">{{ $arsip->ajuan->jenisLayanan->nama }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a href="{{ route('desa.ajuan.show', $arsip->ajuan) }}" class="font-mono text-sm text-primary hover:underline">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <a href="{{ Storage::disk('public')->url($arsip->file_path) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-success text-white text-sm font-medium rounded-btn hover:bg-green-600 transition-all shadow-sm">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center text-muted">
                                    ...
                                    <p class="font-medium">Belum ada Surat Rekomendasi Terbit</p>
                                    <p class="text-sm mt-1">Surat Rekomendasi dari Bupati akan muncul di sini jika ajuan Anda telah selesai diproses oleh Dinpermasdes.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse

    ...
    <body class="font-sans antialiased bg-background text-ink flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-primary text-white flex flex-col transition-all duration-300 flex-shrink-0 relative z-20">
            <div class="h-16 flex items-center px-6 border-b border-primary-light/30">
                <h1 class="text-2xl font-display font-bold tracking-tight">SID<span class="text-white">mini</span></h1>
            </div>
            
            <nav class="flex-1 overflow-y-auto scrollbar-none py-4 px-3 space-y-1">
            ...
            <div class="p-4 border-t border-primary-light/30">
                <div class="flex items-center px-2 py-2">
                    ...
                    <div class="ml-3 truncate">
                        ...
                        <p class="text-xs text-white/70 truncate">{{ auth()->user()->role == 'super_admin' ? 'Dinpermasdes' : auth()->user()->desa->nama_desa ?? 'Operator Desa' }}</p>
                    </div>
                </div>
                <form method="POST" action="{{ route('logout') }}" class="mt-2">
                    @csrf
                    <button type="submit" class="w-full text-left px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/10 rounded-btn transition-colors">
                        Logout
                    </button>
                </form>
            </div>
